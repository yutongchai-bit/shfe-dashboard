...
# tolerant premium parser: commas/semicolons/tabs/spaces, Y-M-D or M/D/YYYY, BOM ok
import re as _re
_pr = {}
for _ln in open(f'{D}/premium.csv', encoding='utf-8-sig'):
    _p = _re.split(r'[\s,;\t]+', _ln.strip())
    if len(_p) < 2:
        continue
    _dm = _re.match(r'^(\d{4})[-/.](\d{1,2})[-/.](\d{1,2})$', _p[0].strip('"\''))
    if not _dm:
        _dm2 = _re.match(r'^(\d{1,2})[/.](\d{1,2})[/.](\d{4})$', _p[0].strip('"\''))
        if _dm2:
            _key = f'{_dm2.group(3)}-{int(_dm2.group(1)):02d}-{int(_dm2.group(2)):02d}'
        else:
            continue
    else:
        _key = f'{_dm.group(1)}-{int(_dm.group(2)):02d}-{int(_dm.group(3)):02d}'
    try:
        _pr[_key] = float(_p[1].replace(',', ''))
    except ValueError:
        continue
prem = pd.Series(_pr, name='premium').sort_index()
prem.index.name = 'date'
...
# current cycle: TRUE alignment — last 3 sessions before delivery month + ALL sessions into it
# (tags extend past I+3 as delivery progresses; each point stays frozen at its real date)
cur_short = sm_short.dropna().sort_index()
_pcur = 'CU' + prompt_ym(cur_short.index[-1]) if len(cur_short) else None
# use the prompt contract of the latest session; its delivery month:
_dmc = f'20{_pcur[2:4]}-{_pcur[4:6]}' if _pcur else None
pre_c = cur_short[cur_short.index < _dmc].iloc[-3:] if _dmc else cur_short.iloc[:0]
into_c = cur_short[cur_short.index >= _dmc] if _dmc else cur_short.iloc[:0]
cur_base = pre_c.iloc[0] if len(pre_c) == 3 else (cur_short.iloc[-6] if len(cur_short) >= 6 else np.nan)
cur_aligned = pd.concat([pre_c, into_c])
cur_norm = [round(float(v / cur_base), 3) for v in cur_aligned.values] if pd.notna(cur_base) and cur_base else []
PTAGS = ['P-3', 'P-2', 'P-1'] + [f'I+{i}' for i in range(1, max(len(into_c), 3) + 1)]
hist_p = list(hist) + [np.nan] * (len(PTAGS) - len(TAGS))  # Sina archive only reaches I+3
# extend the average beyond I+3 from EM-era full-daily cycles (since Dec-2025), normalized to P-3
_s_all = sm_short.dropna().sort_index()
_first = _s_all.index[0]
em_curves = []
_ym = int(_pcur[2:6]) if _pcur else None
_cycles_em = []
y0, m0 = 2000 + int(_first[2:4]) if False else int(_first[:4]), int(_first[5:7])
yy, mm = y0, m0 + 2
while (yy % 100) * 100 + mm < (_ym if _ym else 0):
    _cycles_em.append(f'{yy % 100:02d}{mm:02d}')
    mm += 1
    if mm > 12: mm, yy = 1, yy + 1
_smc = mp[(mp.metal == 'CU') & (mp.member.isin(SMELTER))]
for ym in _cycles_em:
    dmx = f'20{ym[:2]}-{ym[2:]}'
    endx = f'{dmx}-16'
    _g = _smc[_smc.contract == 'CU' + ym].groupby('date')['short_oi'].sum().sort_index()
    prex = _g[_g.index < dmx].iloc[-3:]
    intox = _g[(_g.index >= dmx) & (_g.index < endx)].iloc[:len(PTAGS) - 3]
    if len(prex) < 3 or len(intox) < 3 or not prex.iloc[0] or prex.iloc[0] < 500:
        continue
    v = list(prex.values) + list(intox.values)
    v += [np.nan] * (len(PTAGS) - len(v))
    em_curves.append([x / prex.iloc[0] if pd.notna(x) else np.nan for x in v])
if em_curves:
    hist_em = np.nanmean(np.array(em_curves, dtype=float), axis=0)
    # splice continuously: scale EM extension so it matches the 23-cycle average at I+3
    scale = (hist[5] / hist_em[5]) if len(hist_em) > 5 and pd.notna(hist_em[5]) and hist_em[5] else 1.0
    for i in range(len(TAGS), len(PTAGS)):  # only fill where Sina avg is absent
        if i < len(hist_em) and pd.notna(hist_em[i]):
            hist_p[i] = float(hist_em[i] * scale)
n_em = len(em_curves)
print(f'  extended avg I+4.. from {n_em} EM-era cycles: {[round(float(v),3) if pd.notna(v) else None for v in hist_p[len(TAGS):]]}')
pace_cur = (cur_aligned.iloc[-1] / cur_aligned.iloc[0]) ** (1 / max(len(cur_aligned) - 1, 1)) - 1 if len(cur_aligned) > 1 and cur_aligned.iloc[0] else np.nan
pace_hist = (hist[2] / hist[0]) ** (1 / 2) - 1 if len(hist) > 2 and hist[0] else np.nan
print(f'  current cycle aligned path ({_pcur}, P-3..I+{len(into_c)}): {cur_norm} | pace {pace_cur*100:+.1f}%/session vs hist pre-delivery {pace_hist*100:+.1f}%/session')
...
out = {'surges': surge_list, 'watch': watch, 'accept': accept, 'resid': resid,
       ...
       'pace': {'tags': PTAGS, 'hist': [round(float(v), 3) if pd.notna(v) else None for v in hist_p],
                'cur': cur_norm, 'cur_dates': list(cur_aligned.index), 'n_em': n_em,
